...
intended_types <- list(
  ...
  # Nominal categorical (should be factor)
  factor  = c("marst", "race", "speakeng", "schltype", "degfield",
              "ind1990c", "occ2010c", "region", "hw", "education", "educd"),
...
check_and_fix_types <- function(df, intended, label) {
  ...
  for (v in intended$factor) {
    ...
    actual <- class(df[[v]])
    if (actual != "factor") {
      # coerce to factor so model.matrix handles dummies correctly
      df[[v]] <- as.factor(df[[v]])
    }
  }
...
cat_vars  <- c("marst", "race", "speakeng", "schltype", "degfield",
               "ind1990c", "occ2010c", "region", "hw", "education")
...
# --- Rare category pooling (threshold: < 2% in training data) ---
rare_levels <- lapply(train[cat_vars], function(x) {
...
# --- Enforce consistent factor levels across all three sets ---
# Levels are ALWAYS derived from training data
for (v in cat_vars) {
...
build_matrix <- function(df, ref_df = NULL) {
  # If ref_df supplied, align factor levels
  if (!is.null(ref_df)) {
    for (v in cat_vars) {
